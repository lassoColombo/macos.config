return {
  ...
  opts = function()
    vim.keymap.set('n', '<leader>tA', require('ts-node-action').available_actions, { desc = '[T]reesitter available [A]ctions' })
    vim.keymap.set('n', '<leader>ta', require('ts-node-action').node_action, { desc = '[T]reesitter [A]ctions' })
