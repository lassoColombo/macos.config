...
alias ccp = pbcopy
def lf [] {
    ls --all | where type == "file"
}
def L [] {
    ls --all | where type == "dir"
}
# Connects to a host selected from the ~/.ssh/known_hosts file
# using fuzzy search to filter hosts.
def fssh [] {
...
def napc [] {
    nap list | fzf | nap | ccp # ccp is an alias for xclip or pbcopy
}
## python
alias p = python
alias py = python
